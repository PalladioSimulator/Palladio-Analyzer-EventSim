...
public class RJobProcessor {
    ...
    public RJobProcessor(RserveConnection connection) {
    ...
    /**
     * Waits until all jobs left in this processor's queue have been processed. This effectively
    ...
    private class RJobProcessorRunnable implements Runnable {
        @Override
        public void run() {
            RContext context = new RContext(connection.getConnection());
            try {
                EvaluationHelper.evaluateVoid(context.getConnection(), "memory.limit(size=1024000)"); // TODO make
                                                                                  // parameter

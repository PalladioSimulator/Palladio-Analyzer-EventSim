...
public class Controller {
	...
	/**
	 * If a non-aggregated diagram (e.g. a line graph) has more than
	 * DIAGRAM_SIZE_LIMIT values, a warning will be shown to the user because
	 * the diagram output may cause an SVG "Huge input lookup" error.
	 */
	 ...
	 * 
	 * The application controller creates a RController to manage the R
	 * connection and a MeasurementView to open the main SWT dialog.
	 */
...
//		populateMeasuringPoints();
	 ...
	 * 
	 * This method invokes the {@link #getRelatedToMeasuringPoints(String, List)
	 * method.
	 */
	 ...
	 * 
	 * Default values from RDS file will be delegated to the
	 * {@link gui.FilterViewController}.
	 */
	 ...
	 * 
	 * Filter properties from {@link FilterViewController} will be delegated to
	 * the {@link RController}. A new {@link DiagramView} will be generated.
	 */
	public final void plotDiagram() {
		...
		try {
			...
			printFilterInfo(filterSet,
					"Filter Set which is used to plot a " + diagramType + ".");

			// Save the diagram image in temp directory.
			String diagramImageFileExtension = ".svg";
			String diagramImageName = "diagram_";
			File tempFile = File.createTempFile(diagramImageName,
					diagramImageFileExtension);
			...
			if (!diagramType.isAggregating()
					&& diagramSize > DIAGRAM_SIZE_LIMIT) {

				MessageDialog dialog = new MessageDialog(
						Display.getCurrent().getActiveShell(),
						"Huge diagram output", null,
						"The diagram plot will contain more than "
								+ DIAGRAM_SIZE_LIMIT + " values (" + diagramSize
								+ "). This may cause an SVG 'Huge output "
								+ "error'. Do you want to continue or "
								+ "restrict the time span to reduce the number "
								+ "of values used for the diagram plot?",
						MessageDialog.WARNING,
						new String[] { "No, cancel diagram plot",
								"Yes, plot diagram" },
						0);
			...
			try {
				...
				rCtrl.plotDiagram(diagramType, filterSet, diagramImagePath,
						shortDiagramTitle, diagramSubTitle);

				String rCommandString = rCtrl.getRCommandForDiagramPlot(
						diagramType, "dat", filterSet, "img", shortDiagramTitle,
						diagramSubTitle);

				// Open new view to display the diagram.
				DiagramView view = (DiagramView) PlatformUI.getWorkbench()
						.getActiveWorkbenchWindow().getActivePage()
						.showView(DiagramView.ID, diagramTitle,
	 ...
	 * 
	 * This method delegates the available measuring points due to the filter
	 * change from the {@link RController} to the {@link FilterViewController}.
	 */
	...
	private void populateTriggerTypes() {
		...
		String description = "Restrict simulation scope to reduce the number "
				+ "of available trigger instances (max. "
				+ MAX_NUMBER_OF_TRIGGER_INSTANCES_TO_SHOW
				+ " instances allowed).";
	 ...
	 * 
	 * This method delegates the available trigger instances due to the filter
	 * change from the {@link RController} to the {@link FilterViewController}.
	 */
	private void populateTriggerInstances() {
		...
		try {
			...
			try {
				...
				numTriggerInstances = rCtrl
						.getNumberOfTriggerInstances(filterSet);
			...
			LOG.trace(
					"::: Number of trigger instances: " + numTriggerInstances);
			...
			if (numTriggerInstances <= MAX_NUMBER_OF_TRIGGER_INSTANCES_TO_SHOW) {
				...
				view.setTriggerInstanceNumber(
						Integer.toString(numTriggerInstances));

				// Set trigger instances.
				List<Entity> triggerInstances = rCtrl
						.getTriggerInstances(filterSet);
				...
				view.setTriggerInstanceNumber(
						Integer.toString(numTriggerInstances), true);
	...
	private final void populateSimulationTimeBounds() {
		...
		view.setTimeSpanDescription(
				"Simulation starts at " + rCtrl.getSimulationTimeMin()
						+ " and ends at " + rCtrl.getSimulationTimeMax());
	...
	private Set<MeasurementFilter> createFilterSet() {
		...
		if (mp.getFirst() != null) {
			...
			filterSet.add(new MeasurementFilter("where.first.id", "==",
					mp.getFirst().getId()));
		...
		if (mp.getSecond() != null) {
			...
			filterSet.add(new MeasurementFilter("where.second.id", "==",
					mp.getSecond().getId()));
		...
		if (triggerInstance != null) {
			filterSet.add(new MeasurementFilter("who.id", "==",
					triggerInstance.getId()));
		}

		// Add assembly context filter.
		if (assemblyContext != null) {
			filterSet.add(new MeasurementFilter("assemblycontext.id", "==",
	...
	private String createDiagramTitle() {
		...
		try {
			diagramTitle += " ("
					+ GUIStrings.getGUIString(GUIStrings.getDiagramTypes(),
							viewCtrl.getDiagramType())
					+ ")";
		} catch (Exception e) {
			LOG.error("Cannot get selected diagram type "
					+ "to create diagram title.");
		}
	...
	private String createShortDiagramTitle() {
		...
		try {
			...
			diagramTitle += GUIStrings.getGUIString(GUIStrings.getMetrics(),
					viewCtrl.getMetric());

		} catch (Exception e) {
			LOG.error("Cannot get selected metric "
					+ "to create the diagram title.");
		}
	 ...
	 */
	private void printFilterInfo(Set<MeasurementFilter> filterSet,
			String description) {
		LOG.trace("---------------- Filter Set ------------------");
		LOG.trace(description);
		for (MeasurementFilter filter : filterSet) {
			LOG.trace(filter.getProperty() + " " + filter.getOperator() + " "
					+ filter.getCondition());
		}
		LOG.trace("----------------------------------------------");
	}
	...
	private boolean isConnected() {
		return rCtrl.getConnection() != null
				&& rCtrl.getConnection().isConnected();
	}
	...
	private void checkTimespanValues(int timespanStart, int timespanEnd) {
		if (timespanEnd <= timespanStart) {
			LOG.error("Cannot plot diagram! "
					+ "Time span to-value have to be greater than the "

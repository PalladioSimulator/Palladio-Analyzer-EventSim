...
public final class RController {
	...
	/**
	 * Variable name which is used in R to store data from RDS file as data
	 * table.
	 */
	...
	public void loadLibraries() {
		...
		for (String lib : REQUIRED_LIBRARIES) {
			...
			try {
				...
				LOG.error("Could not load R library " + lib
						+ ". Use 'install.packages('" + lib
	 ...
	 */
	public List<Pair<Entity>> getMeasuringPoints(
			final Set<MeasurementFilter> filterSet) {
		if (getConnection() == null || !getConnection().isConnected()) {
			return Collections.emptyList();
		}
		...
		try {
			...
			evalRCommand("entries <- unique(" + rDataTableVar
					+ "[, list(a=where.first.id, "
					+ "b=where.first.name, c=where.second.id, "
					+ "d=where.second.name)])");
			...
			for (int i = 1; i <= numMeasuringPoints; i++) {
				...
				entryFirstName = evalRCommand("entries[" + i + "]$b")
						.asString();

				entrySecondId = evalRCommand("entries[" + i + "]$c").asString();

				entrySecondName = evalRCommand("entries[" + i + "]$d")
						.asString();

				LOG.trace("Measuring point " + i + " - from id: " + entryFirstId
						+ " - from name: " + entryFirstName + " - to id: "
						+ entrySecondId + " - to name: " + entrySecondName);
	...
	public List<Entity> getTriggerInstances(
		...
		try {
			...
			evalRCommand("instances <- unique(" + rDataTableVar
					+ "[, list(id=who.id, name=who.name)]);");
			...
			for (int i = 1; i <= numInstances; i++) {
				...
				instanceName = evalRCommand("instances[" + i + "]$name")
						.asString();
	...
	/**
	 * Get the simulation time maximum of current data loaded from RDS file and
	 * stored in {@link #CONTENT_VARIABLE}.
	 * 
	...
	/**
	 * Get the simulation time minimum of current data loaded from RDS file and
	 * stored in {@link #CONTENT_VARIABLE}.
	 * 
	...
	public List<Entity> getAssemblyContexts() {
		...
		try {
			...
			evalRCommand("entries <- unique(" + CONTENT_VARIABLE
					+ "[complete.cases(" + CONTENT_VARIABLE + "), list("
					+ "type=assemblycontext.type, "
					+ "id=assemblycontext.id, "
			...
			for (int i = 1; i <= numAssemblyContexts; i++) {
				String name = evalRCommand("entries[" + i + "]$name")
						.asString();
	...
	/**
	 * Plot a diagram by R based on the given filters and stored in the given
	 * path.
	 * 
	 * @param type
	 ...
	 */
	public void plotDiagram(final DiagramType type,
			final Set<MeasurementFilter> filterSet,
			final String diagramImagePath, final String diagramTitle,
		...
		rCmd += getRCommandForDiagramPlot(type, rPlotDataVar, filterSet,
				rImageVar, diagramTitle, diagramSubTitle);

		// Save plot to SVG file.
		rCmd += "ggsave(file='" + diagramImagePath + "', plot=" + rImageVar
				+ ", width=10, height=10);";
	 ...
	 */
	public String getRCommandForDiagramPlot(final DiagramType type,
			final String rPlotDataVar, final Set<MeasurementFilter> filterSet,
			final String rImageVar, final String diagramTitle,
			final String diagramSubTitle) {
		String rCmd = "";

		rCmd += getRCommandForFilteredData(rPlotDataVar, filterSet);
		rCmd += getDiagramSpecificPlotCommand(type, rPlotDataVar, rImageVar,
				diagramTitle, diagramSubTitle);
	 ...
	 */
	private String getDiagramSpecificPlotCommand(final DiagramType type,
		...
		case HISTOGRAM:
			rCmd += getRCommandForHistogramPlot(rPlotDataVar, rImageVar,
					diagramTitle, diagramSubTitle);
			break;

		case POINT_GRAPH:
			rCmd += getRCommandForLineGraphPlot(rPlotDataVar, rImageVar,
					diagramTitle, diagramSubTitle);
			break;

		default:
			throw new RuntimeException("Diagram type is not supported! "
					+ "Diagram type was " + type + ".");
		}
	 ...
	 * 
	 * @param dataVar
	 *            Variable name of the data frame in which the filtered data is
	 *            stored. Use 'rm(dataVar);' in your R command string to delete
	 *            the data frame after working on it.
	 * @param filterSet
	 *            Set of filters which should be applied to the data stored in
	 *            {@link #CONTENT_VARIABLE}.
	 * @return R command as string.
	 */
	private String getRCommandForFilteredData(final String dataVar,
			final Set<MeasurementFilter> filterSet) {
		...
		for (MeasurementFilter filter : filterSet) {
			...
			if (filter.getCondition() == null) { // TODO correct?
				...
				rCmd += CONTENT_VARIABLE + "$" + filter.getProperty()
						+ filter.getOperator() + filter.getCondition();
	 ...
	 * 
	 * @param dataVar
	 *            Variable name of the data table stored in R which includes the
	 *            data for plotting the diagram.
	 * @param imgVar
	 *            Variable name of the R variable which contains the ggplot data
	 *            and is used for exporting the plot as an image.
	 * @param diagramTitle
	 *            Diagram title.
	 * @param diagramSubTitle
	 *            Diagram sub title.
	 * @return R command as string.
	 */
	private String getRCommandForHistogramPlot(final String dataVar,
			final String imgVar, final String diagramTitle,
			final String diagramSubTitle) {

		return imgVar + "=qplot(value, data=" + dataVar
				+ ", geom='histogram') + " + DIAGRAM_THEME + " + "
				+ "ggtitle(expression(atop('" + diagramTitle + "', "
				+ "atop(italic('" + diagramSubTitle + "'), ''))));";
	}
	 ...
	 * @return R command as string.
	 */
	private String getRCommandForLineGraphPlot(final String dataVar,
			final String imgVar, final String diagramTitle,
			final String diagramSubTitle) {
		return imgVar + "=qplot(x=when, y=value, data=" + dataVar
				+ ", geom='point') + " + DIAGRAM_THEME + " + "
				+ "ggtitle(expression(atop('" + diagramTitle + "', "
				+ "atop(italic('" + diagramSubTitle + "'), ''))));";
	}

	/**
	 * Apply a set of filter to the simulation data and store the filtered data
	 * as a data table in a variable with the given name.
	 * 
	 * @param varName
	 ...
	 */
	private void filterDataAndStoreInVariable(final String varName,
			final Set<MeasurementFilter> filterSet) {
		String rCmdFilteredDataTable = getRCommandForFilteredData(varName,
				filterSet);
		try {

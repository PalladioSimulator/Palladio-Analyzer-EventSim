 ...
 * 
 * Delegates communication between plug-ins main controller ({@link Controller})
 * and filter view ({@link FilterView}) and provides some conversion
 * functionality ( e.g. in {@link #getDiagramType()}).
 * 
 * @see FilterView
 * 
...
public final class FilterViewController {
	 ...
	 * 
	 * Store measuring point pairs for updating 'to' measuring points after the
	 * user selects another 'from' measuring point.
	 */
	 ...
	 * 
	 * @param metrics
	 *            List of all available metrics with their technical names (e.g.
	 *            QUEUE_LENGTH).
	 */
	 ...
	 * @see GUIStrings
	 * 
	 * @return Currently selected metric as technical string (e.g.
	 *         QUEUE_LENGTH).
	 * @throws Exception
	 *             If an invalid technical name was used.
	 */
	public String getMetric() {
		return GUIStrings.getTechnicalName(GUIStrings.getMetrics(),
				filterView.getMetric());
	}
	...
	public Pair<Entity> getMeasuringPoints() {
		Pair<Entity> mp = new Pair<>(filterView.getFromMeasuringPoint(),
				filterView.getToMeasuringPoint());
		return mp;
	}
	...
	public void setDiagramTypes(final List<DiagramType> diagramTypes) {
		String[] types = GUIStrings.getGUIStrings(GUIStrings.getDiagramTypes(),
				diagramTypes);
	...
	public DiagramType getDiagramType() {
		return GUIStrings.getTechnicalName(GUIStrings.getDiagramTypes(),
				filterView.getDiagramType());
	}
	...
	/**
	 * Set the available assembly contexts. This method will add the default
	 * option 'all' to the list of possible assembly contexts.
	 * 
	 * @param ctxs
	 ...
	 * @param mps
	 *            List of available measuring point pairs.
	 * @return List of readable strings (name and id) of the related 'to'
	 *         measuring points.
	 */
	private Entity[] getRelatedToMeasuringPoints(
			final String fromMeasuringPointId,
			final List<Pair<Entity>> mps) {

		List<Entity> availableToMeasuringPoints = new ArrayList<>();

		for (Pair<Entity> mp : mps) {
			if (mp.getFirst().getId().equals(fromMeasuringPointId)
					&& mp.getSecond().getId() != null) {
	...
	public void setRelatedToMeasuringPoints() {
		...
		Entity[] tos = getRelatedToMeasuringPoints(from.getId(),
				measuringPoints);
